...
const MOCK_RELEASE_RESPONSE = {
  tag_name: 'v0.9.0',
  html_url: 'https://github.com/ivan-malinovski/Calino/releases/tag/v0.9.0',
}
...
describe('useUpdateCheck', () => {
  ...
  it('detects when a newer version is available', async () => {
    ...
    expect(result.current.latestVersion).toBe('v0.9.0')
  ...
  it('dismisses the update and persists to localStorage', async () => {
    ...
    expect(result.current.hasUpdate).toBe(false)
    expect(memoryStore.get('calino-dismissed-version')).toBe('v0.9.0')
  })
  ...
  it('shows update again when a newer version appears after dismissal', async () => {
    ...
    vi.mocked(globalThis.fetch).mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: async () => ({ tag_name: 'v0.9.1', html_url: 'https://example.com/v0.9.1' }),
    } as Response)
    ...
    expect(result.current.hasUpdate).toBe(true)
    expect(result.current.latestVersion).toBe('v0.9.1')
  })
